(function() {
  ...
  function main(retries = 0, maxRetries = 30) {
    const skipEdgeMeshCache = window.Rivo?.loy_config?.account_widget_settings?.skip_edgemesh_cache;
    const isEdgeMesh = !skipEdgeMeshCache && (
      window.Rivo?.loy_config?.account_widget_settings?.cache_app_type == "edgemesh" ||
      window.edgemesh ||
      window.EDGEMESH
    );
  ...
  function edgemeshRivoAppInit() {
    fetch(`${window.location.origin}/em-cgi/dynamic${window.location.pathname}${window.location.search}`, {
      credentials: "include"
    })
    .then(response => {
    ...
    .then(htmlArray => {
      ...
      if (rivoAppEmbedChunk) {
        ...
        if (rivoAppEmbedInit) {
          ...
          const freshDocument = parser.parseFromString(rivoAppEmbedChunk, 'text/html');
          const freshElement = freshDocument.querySelector('#rivo-js-customer-init');
          const newElement = document.createElement('script');
          Array.from((freshElement || rivoAppEmbedInit).attributes).forEach(attr => {
            newElement.setAttribute(attr.name, attr.value);
          });
          newElement.innerHTML = freshElement ? freshElement.innerHTML : rivoAppEmbedChunk;
          rivoAppEmbedInit.replaceWith(newElement);
        }
